...
class Action(ABC):
    def __init__(self, id: str, buttonName: str, toolTip: str, commandClass, resource: Optional[str] = None) -> None:
        super().__init__()
        self.id = id
...
class AddIn(ABC):
    ...
    def addToUi(self):
        ...
        for action in self._actions:
            resource = 'resources/ui/create_button' if action.resource is None else action.resource

            commandDefinition = self.fusionUI.commandDefinitions.addButtonDefinition(
                f"{prefix}_{action.id}", action.buttonName, action.toolTip, resource)
            ...
            buttonControl.isPromotedByDefault = action.hasUi()
            buttonControl.isPromoted = action.hasUi()
